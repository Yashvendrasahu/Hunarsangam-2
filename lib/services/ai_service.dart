...
class AiService {
  ...
  /// Invokes Python FastAPI / Supabase AI Service: `fair-price-estimator`
  Future<AiPricingResult> calculateFairPrice({
    ...
    if (kDebugMode) {
      print('[AiService] Calling Python FastAPI fair-price-estimator endpoint...');
    }

    try {
      final aiResult = await SupabaseService().calculateFairPriceAI(
        craftCategory: category,
        materialCost: 85.0 + (diameterIn > 14.0 ? 30.0 : 0.0),
        laborHours: 5.5,
        artisanTier: 'artisan',
        complexity: 'medium',
      );

      final suggested = (aiResult['artisan_recommended_payout'] as num?)?.round() ?? 280;
      final rawCost = (aiResult['material_cost_per_unit'] as num?)?.toDouble() ?? 85.0;
      final laborCost = (aiResult['total_labor_cost'] as num?)?.toDouble() ?? 120.0;

      return AiPricingResult(
        rawMaterialsCost: rawCost,
        laborCost: laborCost,
        clusterOverheadCost: 25.0,
        fairProfitCost: 50.0,
        suggestedPrice: suggested,
        minimumPrice: (suggested * 0.85).round(),
        maximumPrice: (suggested * 1.25).round(),
        clusterBenchmark: 'HunarSangam AI & ODOP Rate Card',
      );
    } catch (_) {
      // Fallback
      double raw = 85.0;
